...
from nltk.stem import PorterStemmer   # PorterStemmeris imported from nltk.stem
...
stemmer = PorterStemmer()             # creates a stemmer objebct of class PorterStemmer
...
for i in range(len(sentences)):       #iterating through the list , each sentence separated . 
    ...
    words=[stemmer.stem(word) for word in words if word not in set(stopwords.words('english'))]  #if word is in stopwords list don't fo anything , otherwise do stemming
